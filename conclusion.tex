...
%%%% REPEAT THIS IN THIE SECTION
% It should be noted that while the injected traffic provides a useful approximation
% of how reconnaissance attacks may appear in the different customer environments,
% not all effects of an attack executed directly inside the companies’ networks are
% reproduced, such as device-specific responses, firewall behavior, or operational side
% effects

\section{Future Work}
\textcolor{blue}{Future work: evaluate more environments on our selected feature sets and models and then maybe select one that is not the best for one environment but that preforms better overall.}
