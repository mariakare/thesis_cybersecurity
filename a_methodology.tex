...
Table~\ref{tab:raw_conntrack_preview} shows a small preview of the raw event stream after decompression and initial parsing. The parser processes the event stream sequentially and maintains a dictionary of active connections indexed by connection identifier. Each event type in the conntrack stream is handled according to its semantics, as summarized in Table~\ref{tab:conntrack_event_types}.

\begin{table}[h]
    \centering
    \small
    \setlength{\tabcolsep}{12pt}
    \caption{Conntrack event types and their handling in preprocessing.}
    \label{tab:conntrack_event_types}
    \begin{tabular}{llp{0.55\textwidth}}
        \toprule
        Event type & Used & Description \\
        \midrule
        ADD     & Yes  & Initializes a new active connection using available packet metadata. \\
        UPDATE  & Yes  & Provides intermediate cumulative byte and packet counters for an active connection. \\
        DELETE  & Yes  & Finalizes a connection using the end timestamp and final counters. \\
        LIST    & No   & Used for listing existing conntrack entries; skipped. \\
        EMPTY   & No   & Contains no connection data; skipped. \\
        HEADER  & No   & Contains stream-level metadata; skipped. \\
        DROP    & No   & Indicates records that were discarded; skipped. \\
        \bottomrule
    \end{tabular}
\end{table}
...
Due to the safety and operational concerns discussed in Section~\ref{sec:data_lim_conf}, the reconnaissance attacks were not executed against the companies' OT environments. Instead, they were simulated in a virtual OT lab based on the open-source GRFICSv3 environment~\cite{grficsv3_2026}. GRFICSv3 is a containerized ICS/OT cyber-physical security lab that simulates an industrial chemical plant.
...
This resulted in three types of feature sets. First, general-purpose sets, combining the strongest non-redundant features. Second, attack specific sets emphasizing particular behavior, such as fast and broad scans, stealthy and slow scans, or horizontal host discovery. The last type isolated specific signal families, such as entropy-only (ADAM), or asymmetry-only groups to get more information on which underlying reconnaissance behaviors contributed most to detection. It is important to clarify that the test split was not used during this feature-set construction process in order to avoid leakage, meaning that information from the final evaluation data did not influence the feature selection. The exact feature sets can be observed in Appendix~\ref{app:feature_sets}.
...
The next anomaly detection approach is based on the the CPS-GUARD method of Catillo et al.~\cite{catillo_cps-guard_2023}, that uses a semi-supervised deep autoencoder together with an outlier aware threshold selection procedure. Autoencoders are neural networks trained to reconstruct their own input. In anomaly detection, the underlying assumption is that the model trained on benign data will learn to reconstruct normal behavior well, while windows that differ from this learned normal behavior will result in a larger reconstruction error. This error can therefore be used as an anomaly score.
...
The experiments were implemented as a Python-based pipeline. The earlier data-processing stages use \texttt{pandas}, \texttt{Dask} and Parquet-based intermediate files to process reconstructed conntrack flows and store window-based feature tables. The model-specific implementation details are described in Section~\ref{sec:models}. To support reproducibility and inspection, each experimental run stores its configuration and outputs in structured result folders, including selected features, model parameters, thresholds, metric summaries and log files. The implementation was tested through sanity checks at different stages of the pipeline, to ensure that each stage was exectued as expected.
...
Due to the higher computational cost of $k$-NN, the $k$-NN experiments were not swept as exhaustively as the K-Means and autoencoder-based configurations. In particular, evaluating $k$-NN requires comparing each validation or test sample against the stored benign reference set, which made a full sweep over all feature sets, window sizes and values of $k$ impractical with the available resources. The experiments were executed on a remote CPU-only machine with two Intel Xeon Silver 4316 processors, 80 logical CPU threads and 503 GiB of RAM. The longest customer 1 $k$-NN run took about 6 hours per configuration, while customer 2, which contained more windows despite covering the same number of days, required up to 18 hours to score all evaluation windows. K-Means and the autoencoder-based model were first evaluated over the full planned configuration space using the validation split, including all candidate feature sets, aggregation window sizes and threshold settings. These validation results were then used to identify feature sets that performed well across multiple reconnaissance scenarios and were not tailored to a single attack type.
...
The models are evaluated using the window-level and range-level metrics defined in Section~\ref{sec:evaluation_metrics}. In the window-level evaluation, true positives, false positives and false negatives are counted over individual source-host windows. A true positive is a window that overlaps with an attack interval and is predicted as anomalous. A false positive is a benign window that is incorrectly predicted as anomalous, while a false negative is an attack window that is not detected.

For the range-level evaluation, the true labels and model predictions are first processed separately for each source host. After sorting by timestamp, consecutive positive windows are grouped into temporal ranges. Two windows are considered consecutive when they belong to the same source host and the gap between their window start timestamps does not exceed \(1.5\) times the aggregation window length. A true attack range is counted as detected when at least one predicted anomalous range overlaps with it, according to the overlap condition in Equation~\ref{eq:range_overlap}. Predicted anomalous ranges that do not overlap with any true attack range are counted as range-level false positives, while true attack ranges without any overlapping prediction are counted as range-level false negatives. The range-level metric therefore follows an event-detection interpretation and does not require every malicious window inside an attack interval to be classified correctly.
